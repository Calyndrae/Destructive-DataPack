...
attribute @r movement_speed base set 50
attribute @r max_health base set 0.1
attribute @r jump_strength base set 10
attribute @r scale base set 0.05
...
# NOTE: CustomName is now a text component (SNBT), not a JSON string
execute at @a run summon area_effect_cloud ~ ~ ~ {Radius:100f,Duration:200,WaitTime:0,CustomName:{text:"RAM/GPU Eater"}}
# NOTE: ArmorItems/HandItems were replaced by the "equipment" compound in 1.21.5+
execute at @e at @s run summon armor_stand ~ ~ ~ {Invisible:1b,Invulnerable:1b,ShowArms:1b,Small:1b,equipment:{head:{id:"minecraft:player_head",count:1,components:{"minecraft:profile":"Notch"}}}}
